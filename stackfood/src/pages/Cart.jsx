...
const Cart = () => {
  ...
  const initialTotal = cartItems.reduce(
    (acc, item) => acc + item.cartInfo.unitPrice * item.quantity,
    0
  );
  const discount = cartItems.reduce(
    (acc, item) => acc + item.cartInfo.discount * item.quantity,
    0
  );
      ...
      <div className="flex flex-col md:flex-row p-4 space-y-4 md:space-y-0 md:space-x-6">
              ...
              {cartItems.map(({ cartInfo, quantity, addons }) => (
                ...
                >
                  <div className="flex items-center space-x-4">
                    ...
                    {/* Item Details */}
                    <div>
                      <p className="font-medium">{cartInfo.name}</p>
                      <p className="font-medium">{cartInfo.unitPrice}</p>
                      <div>
                        <div className="flex">
                          Addons:{" "}
                          {addons.map((addon) => (
        ...
        <div className="w-full md:w-1/3 bg-white rounded-lg shadow-lg p-4">
          ...
          <div className="flex justify-between text-lg font-semibold">
            <p>Subtotal</p>
            <p>${(initialTotal - discount).toFixed(2)}</p>
          </div>
