...
const trendsData = [
  {
    ...
    discount: "10% OFF",
    rating:"4.9",
    image: "https://www.shutterstock.com/image-photo/table-filled-dishes-food-set-260nw-2477073165.jpg", // Replace with actual image URL
...
const PopularNearby = () => {
  ...
  const getCardClass = (index) => {
    if (index === activeIndex) {
      return "w-[260px] h-[300px] z-20 transform scale-100 transition-all duration-[800ms] [cubic-bezier(0.4, 0, 0.2, 1)] overflow-hidden";
    } else if (index === activeIndex - 1 || index === activeIndex + 1) {
      return " z-10 transform scale-90 transition-all duration-[800ms] [cubic-bezier(0.4, 0, 0.2, 1)] overflow-hidden";
    } else {
      return " z-0 transform scale-75 transition-all duration-[800ms] [cubic-bezier(0.4, 0, 0.2, 1)] overflow-hidden";
    }
  };
  ...
  return (
    <div className=" mx-20 h-96 bg-gray-100">
      ...
      <div className=" w-full px-20">
        ...
        >
          {trendsData.map((item, index) => (
            ...
            <div className="p-1 relative h-72 dark:bg-black max-w-xs overflow-hidden rounded-lg cursor-pointer transform transition-transform duration-300 " onClick={()=>setPopup(prevPopup=>!popup)} >
              ...
              <img src={item.image} alt={item.title} className="w-full h-40 rounded-md object-cover transform transition-transform duration-300 hover:scale-125 z-0" />
              <FaPlus className="absolute bottom-2 right-4 text-red-400 bg-white rounded-full p-1 text-[20px]"/>
              </div>

              <div className="p-2">
                            <p className="text-sm font-normal text-gray-400">{item.restaurant}</p>
